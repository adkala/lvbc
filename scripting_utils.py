...
def make_horizonlstm_config(
    ...
    **kwargs,
    ...
    # optimizer
    if optimizer == "adamw":  # technically wrong, adam and adamw different in pytorch
    ...
    training_loop = lambda datasets: utils.cont_training_loop(
        model,
        optimizer,
        criterion,
        datasets,
            ...
            with open(fp, "rb") as f:
                datasets.append(
                    datasets_utils.ContDataset(
                        pickle.load(f), window=window, horizon=horizon, delta_p=delta_p
                    )
                )
