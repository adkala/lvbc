...
def cont_training_loop(
    model, optimizer, criterion, datasets, device, batch_size=256, comp_error=False
):
    total_loss = 0
    total = 0
    for j in range(len(datasets)):
        ...
        for i, (x, y) in enumerate(
            (
                pbar := tqdm(
                    DataLoader(datasets[j], batch_size=batch_size, shuffle=True),
                    desc="Loss: N/A",
                    leave=False,
                )
            )
            ...
            if comp_error:
                y = compensate_error(
                    y_pred.detach(), y
                )  # does not backprop through compensated vectors
...
def validate_data(config, samples=10):
    ...
    for _ in range(samples):
        ...
        y_pred = config["model"].generate(
            torch.tensor(x[: config["window"]]).float(),
            torch.tensor(x[config["window"] :, 6:]).float(),
        )
        y_pred = (
            y_pred[:, :3].detach().cpu().numpy() * config["validation_datasets"][i].p_r
        )
