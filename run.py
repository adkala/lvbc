...
def make_config(config_file: str) -> dict:
    ...
    config = scripting_utils.get_make_dataset_config(config_kwargs["dataset"])(
        **config_kwargs
    )
    sample_point = config["train_datasets"][0][0]  # for shape matching
    config.update(
        scripting_utils.get_make_model_config(config_kwargs["model"])(
            input_size=sample_point[0].shape[-1],
            output_size=sample_point[1].shape[-1]
            * (
                2 if config_kwargs["loss_function"] == "gaussian" else 1
            ),  # heteroscedastic
            **config_kwargs,
        )
    )
    ...
    config["name"] = (
        ...
        + "_"
        + f'lr{config_kwargs["learning_rate"]}-l{config_kwargs["num_layers"]}-h{config_kwargs["hidden_size"]}-{config_kwargs["loss_function"]}'
        + "_"
...
def run_training_loop(config, load_model=None):
    ...
    for e in range(epochs + 1, config["epochs"]):
        ...
        if e % config["save_iter"] == 0 or True:
            # validate
            ...
            torch.save(
                ...
                f'models/{config["name"]}/{config["name"]}.p',
            )
